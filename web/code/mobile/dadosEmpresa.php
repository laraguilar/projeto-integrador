...
if ($isAuth) {
	...
	$query1 = mysqli_query($conn, "SELECT * from empresa where idEmpresa = '$idEmpresa'");
    ...
    if (mysqli_num_rows($query1) > 0) {
        ...
        while ($row = mysqli_fetch_array($query1)) {
            ...
            $empresa = array();
            $empresa["nomEmpresa"] = $row["nomEmpresa"];
            $empresa["email"] = $row["email"];
            $empresa["cpfCnpj"] = $row["dscCpfCnpj"];
            $empresa["telefone"] = $row["telefone"];
            
            $query2 = mysqli_query($conn, "SELECT * FROM imagem WHERE idEmpresa = '$idEmpresa'");
            if(mysqli_num_rows($query2) > 0){
                
                while($row = mysqli_fetch_array($query2)){
                    $empresa['img'] = $row['nome'];
                }
            }
